...
void SetupGsm(){
    ...
    if(DEBUG_OUT) Serial.println(F("[Custom4G] Starting GSM Setup"));
    TinyGsmAutoBaud(SerialAT, GSM_AUTOBAUD_MIN, GSM_AUTOBAUD_MAX);
    delay(200);
    if(DEBUG_OUT) Serial.println(F("[Custom4G] Initializing modem..."));
    modem.restart();

    String modemInfo = modem.getModemInfo();
    if(DEBUG_OUT) Serial.print(F("[Custom4G] Modem Info: "));
    ...
    if(DEBUG_OUT) SerialMon.println(F("[Custom4G] Waiting for network..."));
    if (!modem.waitForNetwork()) {
        if(DEBUG_OUT) SerialMon.println(F("[Custom4G] fail"));
        delay(200);
        return;
  }
  if(DEBUG_OUT) SerialMon.println(F("[Custom4G] success"));

  if (modem.isNetworkConnected()) {
    if(DEBUG_OUT) SerialMon.println(F("[Custom4G] Network connected"));
  }

#if TINY_GSM_USE_GPRS
  // GPRS connection parameters are usually set after network registration
    if(DEBUG_OUT) SerialMon.print(F("[Custom4G] Connecting to "));
    SerialMon.print(apn);
  ...
  if (modem.isGprsConnected()) {
    if(DEBUG_OUT) SerialMon.println(F("[Custom4G]GPRS connected"));
  }
...
void ConnectToServer(){
   ...
   if(DEBUG_OUT) SerialMon.print(F("[Custom4G] Connecting to "));
    ...
    if (!client.connect(host, port_m)) {
       if(DEBUG_OUT) SerialMon.println(F("[Custom4G] Connection request failed"));
        delay(1000);
        return;
    }
   if(DEBUG_OUT) SerialMon.println(F("[Custom4G] Connected"));
  ...
  // Make a HTTP GET request:
    ...
    if(DEBUG_OUT) Serial.println("[Custom 4G: ]"+ String(url));
    if(DEBUG_OUT) SerialMon.println(F("Upgrading connection to websockets!"));
...
void gsm_Loop(){
    ...
    if(!client.connected()){
        Serial.println("[Custom 4G] Client is disconnected. Trying to reconnect");
        ...
        Serial.println("[Custom 4G] Client connected");
    }
  ...
  //  delay(1000);
   ...
   // Serial.println("Index 2--->"+String(index3));
    if(index1 != -1 && index2 != -1){
        ...
        Serial.println("[Custom 4G] Wsc_Txt: "+DataString);
         
        payload = (uint8_t *)DataString.c_str();
    
        if (!processWebSocketEvent(payload, DataLen)) { //forward message to OcppEngine
            if(DEBUG_OUT) Serial.print(F("[Custom 4G] Processing WebSocket input event failed!\n"));
        }  
...
/*void gsmOnEvent(){
 ...
 //Commented this section
   ...
   if(DEBUG_OUT) Serial.println("[CustomGsm]Indexes are: ");
    ...
    if (!processWebSocketEvent(payload, DataLen)) { //forward message to OcppEngine
       if(DEBUG_OUT) Serial.print(F("[CustomGsm] Processing WebSocket input event failed!\n"));
      }   
   }
...
if(!client.connected()){
   ...
   if(DEBUG_OUT) Serial.println("[Custom 4G] Client is not connected");
    return false;
}
...
#endif
  if(DEBUG_OUT)  Serial.println("[Custom 4G-frame] Return : "+String(ret));
    return ret;
}
